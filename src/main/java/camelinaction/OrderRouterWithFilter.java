	...
	public static void main(String args[]) throws Exception {
		...
		context.addRoutes(new RouteBuilder() {
			...
			public void configure() {
				...
				from("jms:xmlOrders").filter(xpath("/book[1]")).process(new Processor() {
					public void process(Exchange exchange) throws Exception {
						...
						Book book = tc.convertTo(Book.class, exchange.getIn().getBody(Book.class));

						System.out.println("Title = " + book);

						System.out.println("Book Name =" + exchange.getIn().getBody().getClass());
						//
					}
				});
		...
		context.getTypeConverterRegistry().addTypeConverter(Book.class, Object.class, new MyBookTypeConverter());
		context.start();
		Thread.sleep(10000);
